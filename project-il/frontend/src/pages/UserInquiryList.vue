    ...
    <div class="inquiry-list-container">
      ...
      <div v-else class="card-list">
        ...
        >
          ...
          <div class="info">
            <span><strong>{{ $t('inquiry.common.category') }}</strong> {{ $t('inquiry.category.etc') }}</span>
...
const fetchInquiries = async () => {
  try {
    ...
    alert(t('inquiry.error.fetch_failed'));
  }
};
...
const goToWrite = async () => {
  const token = localStorage.getItem('user_token');
  try {
    const res = await axios.get('/users/inquiry/pending', {
      headers: { Authorization: `Bearer ${token}` }
    });

    if (res.data.hasPending) {
      alert(t('inquiry.error.pending_exists'));
    } else {
      router.push('/support/inquiry/write');
    }
  } catch (err) {
    console.error('Pending check failed:', err);
    alert(t('inquiry.error.check_failed'));
  }
};
