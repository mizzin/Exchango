<template>
 ...
 <!-- 출금금액입력-->
    <div class="form-group">
      <label>{{ $t('withdraw.inputKrw') }}</label>
      <input
        type="number"
        v-model.number="form.amount "
        :placeholder="$t('withdraw.inputKrwPlaceholder')"
        @input="calculateConvertedAmount"
      />
    </div>
 <!-- 총전송 -->
    <div class="form-group">
      <label>{{ $t('withdraw.totalSend') }} ({{ form.currency }})</label>
    ...
    <div class="form-group" v-if="form.amount > 0">
      <details>
        <summary class="detail-summary">{{ $t('withdraw.viewDetails') }}</summary>
         <ul class="details">
          <li>{{ $t('withdraw.rate') }}: {{ exchangeRateDisplay }} {{ form.currency }}</li>
...
const fetchExchangeRate = async () => {
  ...
  } else if (form.currency  === 'PHP') {
    // 원화 → USD → PHP
    const krwToUsd = 1 / rates['KRW']
    const usdToPhp = rates['PHP']
    exchangeRate.value = krwToUsd * usdToPhp
  } else if (form.currency === 'USDT') {
    const krwToUsd = 1 / rates['KRW']
    exchangeRate.value = krwToUsd // USDT ≈ USD
  } else {
    return alert('환율 계산 실패')
  }
